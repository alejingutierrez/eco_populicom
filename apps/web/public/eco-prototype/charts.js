...
function PRMap({ municipalities, accessor, colorFn, onMunicipalityClick }) {
  ...
  // Leaflet renders half-blank tiles if the container resizes after mount
  // (mobile drawer opening, orientation change, breakpoint reflow). Re-run
  // invalidateSize on resize and once shortly after mount when layout settles.
  React.useEffect(() => {
  ...
  if (typeof window !== 'undefined' && !window.L) {
    return (
      <div style={{ height: 'clamp(300px, 56vh, 460px)', borderRadius: 8, background: 'var(--canvas-2)', border: '1px solid var(--hairline)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-3)', fontSize: 12 }}>
        Cargando mapa…
      </div>
    );
  }
