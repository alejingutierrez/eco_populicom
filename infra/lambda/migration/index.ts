...
export const handler = async (event: { action?: string; query?: string; queryIds?: number[]; limit?: number; agencySlug?: string }): Promise<{ statusCode: number; body: string }> => {
  ...
  try {
    ...
    if (action === 'create-chat-schema') {
      // Crea las tablas del chat contextual: chat_conversations + chat_messages.
      // El dueño del hilo se identifica por Cognito sub (user_sub), no por FK a
      // users (evita depender del aprovisionamiento JIT de esa fila).
      // Idempotente — IF NOT EXISTS en todo. El route también las auto-crea
      // (ensureChatSchema), así que esta acción es el camino explícito/manual.
      const stmts = [
